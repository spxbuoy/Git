...
# --- ADMIN STATE HANDLER ---
# Also replaced filters.create with filters.private & filters.text and check inside
@app.on_message(filters.private & filters.text)
async def admin_states_handler(_, msg: Message):
    if not msg.from_user:
        return
    uid = msg.from_user.id
    # Only handle admin-specific states here.
    # If the user doesn't have an admin-related state, ignore.
    state = user_states.get(uid)
    if not state:
        return
...
if __name__ == "__main__":
    ...
    try:
        ...
        traceback.print_exc()
        # keep process alive long enough to read logs if run manually
        # (optional) you could sleep or exit with non-zero status
        raise
