...
def load_data():
    ...
    if os.path.exists(DATA_FILE):
        try:
            ...
            print("Failed to load data.json, creating new one:", e)
            save_data()
    else:
        save_data()
...
# --- Notification helper (HTTP to Bot API) ---
# Used to notify ADMINS if app.run() crashes before Pyrogram client can send DMs.
async def notify_admins_http(text: str):
    ...
    async with aiohttp.ClientSession() as session:
        for admin_id in ADMINS:
            try:
                ...
                print("notify_admins_http failed for", admin_id, e)
...
# --- SINGLE MESSAGE HANDLER FOR BOTH USER & ADMIN STATES ---
...
async def message_handler(_, msg: Message):
    ...
    # Admin-only actions are permitted only for admins
    is_admin = uid in ADMINS

    try:
        # Admin actions (if admin)
        if is_admin and action in ("admin_ban_user", "admin_unban_user", "admin_broadcast"):
        ...
        # Non-admin or other user actions:
        if action == "token_add":
            token = text.strip()
            if len(token) < 10:
...
async def show_repos_page(message: Message, token: str, page: int):
    ...
    buttons = []
    for repo in repos:
        buttons.append([InlineKeyboardButton(repo["name"], callback_data=f"download_repo_zip:{repo['owner']['login']}:{repo['name']}")])
    nav_buttons = []
    if page > 0:
...
async def admin_callbacks(_, cq: CallbackQuery):
    ...
    if data_cb == "admin_list_users":
        ...
        if not users_list:
            ...
            return
        lines = []
        for uid_str, udata in users_list[:50]:
...
async def run_bot_and_notify():
    """Start the client, send healthy start message to admins, then idle."""
    started = False
    try:
        await app.start()
        started = True
        print("Pyrogram client started successfully.")
        # Build a startup message with small diagnostics
        now = datetime.now(timezone.utc).astimezone().isoformat()
        total_users = len(data.get("users", {}))
        text = f"✅ Spilux bot started.\nTime: {now}\nUsers registered: {total_users}"
        # Use app to send to admins (client is started)
        for admin in ADMINS:
            try:
                await app.send_message(admin, text)
            except Exception as e:
                print("Failed to send start message to admin", admin, e)
        print("Startup notifications attempted.")
        # Idle - let Pyrogram run until termination (correct usage for pyrogram v2+)
        await idle()
    except Exception as e:
        # If client failed to start or crashed, notify admins via HTTP (works even if client not started)
        tb = traceback.format_exc()
        print("Bot failed to start or crashed. Traceback:\n", tb)
        try:
            # try to send via client if started
            short_tb = tb[:1400]  # truncate to avoid oversize Telegram messages
            if started:
                for admin in ADMINS:
                    try:
                        await app.send_message(admin, f"❌ Bot encountered an error:\n{e}\n\nTraceback:\n{short_tb}")
                    except Exception as ex:
                        print("Failed to DM admin via client:", ex)
                        await notify_admins_http(f"❌ Bot encountered an error and DM failed:\n{e}\n\nTraceback:\n{short_tb}")
            else:
                # client not started – use HTTP notify
                await notify_admins_http(f"❌ Bot failed to start:\n{e}\n\nTraceback:\n{short_tb}")
        except Exception as e2:
            print("Failed to notify admins:", e2)
        # re-raise so external process (pm2/systemd) can handle restarts/logging
        raise
    finally:
        # Ensure we stop the client cleanly if it was started
        if started:
            try:
                await app.stop()
                print("Pyrogram client stopped.")
            except Exception as e:
                print("Error while stopping client:", e)

if __name__ == "__main__":
    print("Loading data...")
    load_data()
    print("Starting bot... (this will notify ADMINS on success/failure)")
    # Run the bot loop
    try:
        asyncio.run(run_bot_and_notify())
